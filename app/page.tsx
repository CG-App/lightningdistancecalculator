...
export default function HomePage() {
  ...
  const howToSchema = {
    ...
    step: [
      {
        "@type": "HowToStep",
        name: "Watch for lightning",
        text: "Tap Start when you see the flash.",
      },
      {
        "@type": "HowToStep",
        name: "Listen for thunder",
        text: "Tap Stop when you hear the boom.",
      },
      {
        "@type": "HowToStep",
        name: "Read the distance",
        text: "Use miles≈seconds/5 or km≈seconds/3.",
      },
    ],
  } as const;
  ...
  return (
    ...
    >
      ...
      {/* Render the client component normally; no dynamic() needed */}
      <Suspense
        fallback={
          <div
            className="p-6 rounded-2xl border border-black/10 dark:border-white/10 animate-pulse"
            aria-live="polite"
          >
            Loading…
          </div>
      ...
      {/* Optional crawlable copy below the app */}
      <section
        className="mt-8 prose prose-neutral dark:prose-invert max-w-none"
        aria-label="About the calculator"
      >
        <h2 className="!mt-0">How The Lightning Distance Calculator Works</h2>
        <p>
          The calculator uses the classic flash-to-bang method. Sound travels
          roughly <strong>1 mile every ~5 seconds</strong> (or{" "}
          <strong>1 km every ~3 seconds</strong>). Time the delay between the
          flash and the thunder, and the tool converts that timing into
          distance across multiple units.
        </p>
        <p className="text-sm opacity-80">
          <strong>Safety:</strong> This tool is informational only. Lightning
