// components/LightningDistanceCalculator.tsx
"use client";

import React, { useEffect, useMemo, useRef, useState } from "react";

/**
 * Lightning Distance Calculator – React Widget (uniform light/dark)
 * - Client component
 * - No page-wide backgrounds; the card looks the same in both themes.
...
function milesFromSeconds(seconds: number): number {
  return seconds / 5;
}
function kmFromSeconds(seconds: number): number {
  return seconds / 3;
}
...
function trendFromSeries(
  values: number[],
  deadbandMi = 0.1
): "closer" | "further" | "steady" | "insufficient" {
...
export default function LightningDistanceCalculator({
  ...
  const onSecondsChange = React.useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (isValidSeconds(v)) {
        setSecondsInput(raw);
        if (!isTiming) setShowResult(true);
      } else {
        setSecondsInput("");
        setShowResult(false);
      }
  ...
  const trendLabel =
    trend === "closer" ? "Getting Closer" : trend === "further" ? "Moving Away" : trend === "steady" ? "Holding Steady" : "Need More Data";
    ...
    <div className="w-full flex items-start justify-center py-6 sm:py-12">
      <div className="ldc-card mx-auto w-full max-w-xl pt-3 px-5 pb-5 sm:pt-4 sm:px-8 sm:pb-6 md:pt-5 md:px-8 md:pb-6 rounded-2xl border shadow-xl">
        ...
        <div className="relative grid place-items-center min-h-[56px] sm:min-h-[64px]">
          <div
            className={`absolute inset-0 flex items-center justify-center motion-safe:transition-all motion-safe:duration-300 ease-out ${
              showCongrats ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-2"
            }`}
          >
            <div className="w-full inline-flex items-center gap-2 px-3 py-2 rounded-md bg-indigo-100 text-indigo-900 font-semibold shadow-sm text-center text-sm justify-center overflow-hidden whitespace-normal break-words">
              <span className="truncate">⭐ You&rsquo;re a Storm Watcher!</span>
              <button
                onClick={() => setShowCongrats(false)}
                className="ml-2 text-indigo-700 hover:underline text-xs flex-shrink-0"
          ...
          <p
            className={`text-sm sm:text-base text-gray-600 text-center motion-safe:transition-all motion-safe:duration-300 ease-out ${
              showCongrats ? "opacity-0 -translate-y-2" : "opacity-100 translate-y-0"
            }`}
          >
            Tap <strong>Start</strong> when you see lightning, <strong>Stop</strong> when you hear thunder. Repeat to track the storm&rsquo;s trend.
          </p>
        </div>
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2">
          {!isTiming ? (
            <button className="px-6 py-3 sm:px-5 sm:py-2.5 rounded-xl bg-emerald-600 text-white font-semibold shadow-sm active:scale-[.99] whitespace-nowrap" onClick={startTimer}>
              Start
            </button>
          ) : (
            <button className="px-6 py-3 sm:px-5 sm:py-2.5 rounded-xl bg-rose-600 text-white font-semibold shadow-sm active:scale-[.99] whitespace-nowrap" onClick={stopTimer}>
              Stop
            </button>
          )}
          {/* Units dropdown */}
        ...
        {hasDistance && (
          ...
          >
            ...
            {runsMi.length >= 2 && (
              <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm">
                ...
                <div className="rounded-xl border border-gray-200 bg-white/60 px-3 py-2 text-center">
                  ...
                  <div className={["flex justify-center items-center gap-2 font-semibold text-center text-[1rem] sm:text-[1.125rem]", trendColor].join(" ")}>
            ...
            <div
              className={["mt-3 transform motion-safe:transition-all motion-safe:duration-300 ease-out", revealBanner ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-2 pointer-events-none select-none"].join(" ")}
              ...
              {safety === "danger" && (
                <div className="rounded-xl p-3 sm:p-3.5 bg-red-100 text-red-900 border border-red-200 font-semibold shadow-sm text-center">
                  ...
                  <div className="text-xs sm:text-sm font-semibold mt-0.5">
                    🚨 Risk Score: <span className="font-bold">Extreme (4/4)</span>
                  </div>
                </div>
              )}
              {safety === "warning" && (
                <div className="rounded-xl p-3 sm:p-3.5 bg-orange-200 text-orange-900 border border-orange-300 font-semibold shadow-sm text-center">
                  ...
                  <div className="text-xs sm:text-sm font-semibold mt-0.5">
                    🚨 Risk Score: <span className="font-bold">High (3/4)</span>
                  </div>
                </div>
              )}
              {safety === "caution" && (
                <div className="rounded-xl p-3 sm:p-3.5 bg-yellow-100 text-yellow-900 border border-yellow-200 font-semibold shadow-sm text-center">
                  ...
                  <div className="text-xs sm:text-sm font-semibold mt-0.5">
                    ⚠️ Risk Score: <span className="font-bold">Moderate (2/4)</span>
                  </div>
                </div>
              )}
              {safety === "clear" && (
                <div className="rounded-xl p-3 sm:p-3.5 bg-green-100 text-green-900 border border-green-200 font-semibold shadow-sm text-center">
                  ...
                  <div className="text-xs sm:text-sm font-semibold mt-0.5">
                    ⚠️ Risk Score: <span className="font-bold">Low (1/4)</span>
                  </div>
                </div>
              )}
            </div>
        ...
        <div className="mt-3">
          <div className="flex items-center justify-between">
            ...
            <button onClick={resetAll} className="text-sm text-gray-700 underline focus:outline-none focus:ring-2 focus:ring-emerald-500 rounded">
              Reset All
            </button>
          </div>
          {showWhy && (
            <div id="why-content" className="mt-2 text-sm text-gray-700 space-y-3">
              <p>
                Sound travels about 343 m/s at 20°C (68°F). That&rsquo;s roughly 1 km every ~3 seconds or 1 mile every ~5 seconds. Temperature,
                humidity, wind, and terrain can change this slightly. Treat this as an estimate.
              </p>
              <p>
                <strong>Disclaimer:</strong> This tool provides an estimate based on sound travel time. Lightning can strike unpredictably. For
                informational purposes only — no guarantee of safety is implied. Always follow official weather advisories.
              </p>
            </div>
          )}
        </div>
      ...
      {/* Styled-JSX: single uniform style (same in light & dark) */}
      <style jsx>{`
        .ldc-card {
...
if (typeof window !== "undefined" && !window.__LIGHTNING_CALC_TESTED__) {
  ...
  const f1 = fixedWindowAverage([10, 8, 6, 4], 3);
  if (Math.abs(f1 - 6) > 1e-9) throw new Error(`fixedWindowAverage failed: got ${f1}`);
  const f2 = fixedWindowAverage([5], 3);
  if (Math.abs(f2 - 5) > 1e-9) throw new Error("fixedWindowAverage single value failed");
  const f3 = fixedWindowAverage([9, 6], 3);
  if (Math.abs(f3 - 7.5) > 1e-9) throw new Error("fixedWindowAverage two values failed");
